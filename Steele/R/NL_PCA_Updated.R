...
# Purpose: PCA for Newfoundland Shelf
...
dt <- fread("../output/NL_assembled.csv")

# Filter to include subset of stations
dt <- dt[transect %in% c("BB", "FC", "SI", "SEGB")]

# Zooplankton names
zoo_names <- c("Temora longicornis", "Scolecithricella minor", "Pseudocalanus spp", "Oithona spp", "Microcalanus spp",
               "Metridia spp", "Metridia lucens", "Metridia longa", "Larvacea", "Hydrozoa(?)", "Gastropoda", "Euphausiacea",
               "Cnidaria", "Calanus hyperboreus", "Calanus glacialis", "Calanus finmarchicus", "Bivalvia") 

colnames(dt)[which(names(dt) == "TEMLON"):which(names(dt) == "BIVCLA")] <- zoo_names
...
###################################
# Columns to use
z_col <- paste(which(names(dt) %in% c("Bivalvia", "Temora longicornis")), collapse = ":")
...
###################################
# Columns to use
e_col_names <- c("Ice end day NL", "Ice area NL",
                "Cfin max yearday S27", "Cfin I+II+III max yearday S27",
                "Chyp max yearday S27", "Chyp I+II+III max yearday S27",
                "Biomass max yearday S27", 
                "Bloom duration Flemish Pass", "Bloom duration Hibernia", 
                "Bloom duration Northeast Nfld Shelf", "Bloom duration Southeast Shoal", 
                "Bloom duration St. Anthony Basin", 
                "Bloom start Flemish Pass", "Bloom start Hibernia", 
                "Bloom start Northeast Nfld Shelf", "Bloom start Southeast Shoal",
                "Bloom start St. Anthony Basin", 
                "SST warming Flemish Pass", "SST warming Hybernia",
                "SST warming Northeast Nfld Shelf", "SST warming Southeast Shoal",
                "SST warming St.Anthony Basin")

# Find in data table
e_col <- which(names(dt) %in% e_col_names)

# Add year
e_col <- c(which(names(dt) == "year"), e_col)

# Pare down table
dt_tmp <- unique(dt[, e_col, with = FALSE])

# Reorder
setcolorder(dt_tmp, c("year", e_col_names))

# List of variables for melting
m <- list(names(dt_tmp)[grepl("Bloom duration", names(dt_tmp))],
          names(dt_tmp)[grepl("Bloom start", names(dt_tmp))],
          names(dt_tmp)[grepl("SST warming", names(dt_tmp))])

# But scale first 
dt_tmp[, (unlist(m)) := lapply(.SD, scale), .SDcols = unlist(m)]

# Melt bloom duration, bloom start, and SST for each region
dt_tmp <- melt(dt_tmp, 
               measure = m, 
               value.name = c("Bloom duration", "Bloom start", "SST warming"))

# Fix column
setnames(dt_tmp, "variable", "region")
dt_tmp$region <- c("Flemish Pass", "Hibernia", "Northeast Nfld Shelf",
                     "Southeast Shoal", "St. Anthony Basin")[dt_tmp$region]

# Perform PCA
phe_pca <- PCA(dt_tmp[, -"region"])

###################################
# PHYSICAL
###################################
y_col <- which(names(dt) %in% c("Deep temp 2J", "Deep temp 3K", "Deep temp 3LNO spring", 
                                "Deep temp 3LNO fall", "Deep temp 3Ps",  
                                "SST 2J", "SST 3K", "SST 3L", "SST 3M", "SST 3N", "SST 3O", "SST 3P",
                                "NAO", "AMO", 
                                "CIL volume Seal Island", "CIL volume White Bay",
                                "CIL volume Bonavista", "CIL volume Flemish Cap",
                                "Ice volume NL"))
...
plot_loadings <- function(PCA_list){
  ...
  var_order <- list(phy = c(4, 5, 3, 6, 2, 7:12, 14, 13, 15:17, 1), 
                    zoo = c(2, 4, 3, 15, 10:12, 17, 13, 14, 16, 9, 7, 1, 6, 5, 8), 
                    phe = c(10, 1:7, 9, 8))
  ...
  # Save figure
  ggsave("../output/NL_loadings.tiff", mygrid, dpi = 300, units = "cm", width = 16, height = 16, compression = "lzw")
}
...
plot_scores <- function(PCA_list){
  ...
  for(i in 1:length(PCA_ext_list)){
    ...
    p[[1 + (i-1)*3]] <- p[[1 + (i-1)*3]] + annotate("text", x = 2001.5, y = min(na.omit(PCA$x) %>% 
                                                                                group_by(year) %>% 
    ...
    p[[2 + (i-1)*3]] <- p[[2 + (i-1)*3]] + annotate("text", x = 2001.5, y = min(na.omit(PCA$x) %>% 
                                                                                group_by(year) %>% 
                                                                                summarise_each(funs(mean)) %>% 
                                                                                select(PC2)) + 
                                                      0.05 * diff(range(na.omit(PCA$x) %>% 
    ...
    p[[3 + (i-1)*3]] <- p[[3 + (i-1)*3]] + annotate("text", x = 2001.5, y = min(na.omit(PCA$x) %>% 
                                                                                group_by(year) %>% 
                                                                                summarise_each(funs(mean)) %>% 
                                                                                select(PC3)) + 
                                                      0.05 * diff(range(na.omit(PCA$x) %>% 
  ...
  ggsave("../output/NL_scores.tiff", mygrid, dpi = 300, units = "cm", width = 16, height = 16 * 45/60, compression = "lzw")
}
...
# Write to file
fwrite(PC_out, "../output/NL_PCs.csv", na = NA)

# Output PCAs to file
names(PCA_list) <- c("NL_PCA_phy", "NL_PCA_zoo", "NL_PCA_phe")
NL_PCA <- PCA_list
save(NL_PCA, file = "../output/NL_PCA.RData")
