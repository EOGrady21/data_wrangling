...
source("Station_Name_Lookup.R")

##--------------------------------------------------------------------------------------------
## read ice data from yearly dat files
ifile1 <- '../Updated Data/Ice_Data_Timing/IceGridOccurrence/'
ifile2 <- '/IceGridOccurrence.dat'

df_data_raw_wload = NULL
for(i in 1999:2019){
  ifile = paste(ifile1, i, ifile2, sep = '')
  add = fread(ifile)
  add = cbind(i, add)
  df_data_raw_wload = rbind(df_data_raw_wload, add)
}

## switch lat and lon and remove date to match old data more easily, convert to data frame
df_data_raw_w = data.frame(df_data_raw_wload[,1], df_data_raw_wload[,3], df_data_raw_wload[,2], df_data_raw_wload[,4], df_data_raw_wload[,6], df_data_raw_wload[,8])
rm(df_data_raw_wload)

## reassign column names
names(df_data_raw_w) <- c("year","lat","lon","day_first","day_last","duration")

## make longitude negative
df_data_raw_w$lon = -df_data_raw_w$lon
...
df_data_filtered_w <- df_data_filtered_w %>%
  dplyr::mutate(., region=Station_Name_Lookup(lon, lat,
                                           name_lookup_file="../Ice_Data_Timing/ICES_6IZPS_PP_Ice_Polygons_names_Modified.csv",
                                             coord_lookup_file="../Ice_Data_Timing/ICES_6IZPS_PP_Ice_Polygons_coordinates_Modified.csv")) %>%
...
##--------------------------------------------------------------------------------------------
## save to RData file
ofile <- "../Updated Data/Ice_Timing_20200330.RData"
