...
dt <- fread("../output/HL2_assembled_updated.csv")

# Remove Cabot Strait
dt <- dt[station != "CSL" | is.na(station)]
...
###################################
# Columns to use
z_col <- paste(which(names(dt) %in% c("Calanus finmarchicus", "Euphausiacea")), collapse = ":")
...
e_col <- which(names(dt) %in% c("SST warming", "Ice end day GSL+SS", "Ice area GSL+SS",
                                "Cfin max yearday HL2", "Cfin I+II+III max yearday HL2",
                                "Zooplankton max yearday HL2", "Bloom start", "Bloom duration"))
...
# PHYSICAL
###################################
y_col <- which(names(dt) %in% c("Bottom temp 4V", "Bottom temp 4W", "Bottom temp 4X",
                                "SST 4Vn", "SST 4Vs", "SST 4W", "SST 4XSS", "SST 4eGoM + BoF",
                                "NAO", "AMO", "St. Lawrence river flux", "CIL volume SS",
                                "Ice volume GSL+SS", "Stratification"))
...
plot_loadings <- function(PCA_list){
  ...
  var_order <- list(phy = c(9:11, 5:8, 13, 4, 12, 3:1), 
                    zoo = c(1, 3, 2, 13, 4:12, 14:20), 
                    phe = c(1, 4, 8, 5:7, 2, 3))
  ...
  # Save figure
  ggsave("../output/SS_loadings.tiff", mygrid, dpi = 300, units = "cm", width = 16, height = 16, compression = "lzw")
}
...
plot_scores <- function(PCA_list){
  ...
  ggsave("../output/SS_scores.tiff", mygrid, dpi = 300, units = "cm", width = 16, height = 16 * 45/60, compression = "lzw")
}
...
# Write to file
fwrite(PC_out, "../output/HL_PCs.csv", na = NA)

# Output PCAs to file
names(PCA_list) <- c("SS_PCA_phy", "SS_PCA_zoo", "SS_PCA_phe")
SS_PCA <- PCA_list
save(SS_PCA, file = "../output/SS_PCA.RData")
