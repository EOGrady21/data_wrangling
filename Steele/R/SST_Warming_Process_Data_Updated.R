...
# read data
input_file <- list.files(path="../data/SST/", pattern="*_sst.stat", full.names=TRUE)
...
for (i_file in seq(1, length(input_file), by=1)) {
  ...
  tmp <- tmp %>% dplyr::select(., date.id, mean_sst, region)
  # rename variable
...
##--------------------------------------------------------------------------------------------
## save to RData file
ofile <- "../Updated Data/SST_Warming_20200320.RData"
