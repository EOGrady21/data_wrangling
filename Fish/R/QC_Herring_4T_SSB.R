...
res_gam_A <- run_gams(predictors_A, "Her_4T_Fall_SSB", 3, 3, 
                      na.omit(herring[, grep("PC|Her_4T_Fall_SSB", names(herring)), with = FALSE]))
...
pdf("../output/QC_herring_4T_autumn_SSB_effect.pdf", width = 14, height = 10)
lapply(top_models_A, 
       FUN = plot_gam_effect, 
...
## Time series
pdf("../output/QC_herring_4T_autumn_SSB_ts.pdf", width = 12, height = 10)
lapply(top_models_A, 
       FUN = plot_gam_ts, 
       data = herring, 
       response = "SSB")
...
## Diagnostics
pdf("../output/QC_herring_4T_autumn_SSB_res.pdf", width = 14, height = 10)
lapply(top_models_A, 
       FUN = examine_gam_res, 
       data = herring)
dev.off()
...
# Run all GAMs: predictors, response variables, # knots, max # terms, data
res_gam_S <- run_gams(predictors_S, "Her_4T_Spring_SSB", 3, 3, 
                      na.omit(herring[, grep("PC|Her_4T_Spring_SSB", names(herring)), with = FALSE]))
...
## Partial residual plots
pdf("../output/QC_herring_4T_spring_SSB_effect.pdf", width = 14, height = 10)
lapply(top_models_S, 
       FUN = plot_gam_effect, 
       ylabel = expression("Effect on SSB"))
dev.off()
...
## Time series
pdf("../output/QC_herring_4T_spring_SSB_ts.pdf", width = 12, height = 10)
lapply(top_models_S, 
       FUN = plot_gam_ts, 
       data = herring, 
       response = "SSB")
...
## Diagnostics
pdf("../output/QC_herring_4T_spring_SSB_res.pdf", width = 14, height = 10)
lapply(top_models_S, 
       FUN = examine_gam_res, 
       data = herring)
dev.off()
