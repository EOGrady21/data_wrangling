...
# Load data
herring <- fread("../data/4R_herring_K_stats.txt")
...
pdf("../output/QC_herring_4R_autumn_condition_effect.pdf", width = 14, height = 10)
lapply(top_models_A, 
       FUN = plot_gam_effect, 
...
## Time series
pdf("../output/QC_herring_4R_autumn_condition_ts.pdf", width = 12, height = 10)
lapply(top_models_A, 
       FUN = plot_gam_ts, 
       data = herring, 
       response = "K")
...
## Diagnostics
pdf("../output/QC_herring_4R_autumn_condition_res.pdf", width = 14, height = 10)
lapply(top_models_A, 
       FUN = examine_gam_res, 
       data = herring)
dev.off()
...
## Partial residual plots
pdf("../output/QC_herring_4R_spring_condition_effect.pdf", width = 14, height = 10)
lapply(top_models_S, 
       FUN = plot_gam_effect, 
       ylabel = expression("Effect on K"))
dev.off()
...
## Time series
pdf("../output/QC_herring_4R_spring_condition_ts.pdf", width = 12, height = 10)
lapply(top_models_S, 
       FUN = plot_gam_ts, 
       data = herring, 
       response = "K")
...
## Diagnostics
pdf("../output/QC_herring_4R_spring_condition_res.pdf", width = 14, height = 10)
lapply(top_models_S, 
       FUN = examine_gam_res, 
       data = herring)
dev.off()
