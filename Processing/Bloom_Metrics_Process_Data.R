...
df_data_raw_w <- data.frame(region=character(0), year=numeric(0), initiation=numeric(0), amplitude=numeric(0), duration=numeric(0), magnitude=numeric(0), label=character(0))

## read seawifs data from excel sheet
input_file <- "~/Projects/ICES_6IZPS_PP/data/Bloom_Metrics/AZMP-seawifs-bloom-params-magnitude.xls"
tmp <- read.xlsx2(input_file, sheetName="AZMP-seawifs-bloom-params", as.data.frame=TRUE,
                  header=FALSE, stringsAsFactors=FALSE, startRow=3, colIndex=seq(1,6),
                  colClasses=c("character",rep("numeric", 5)))
names(tmp) <- c("region", "year", "initiation", "amplitude", "duration", "magnitude")
df_data_raw_w <- rbind(df_data_raw_w,
                       tmp %>% dplyr::mutate(., label="seawifs"))

## read modis data from excel sheet
input_file <- "~/Projects/ICES_6IZPS_PP/data/Bloom_Metrics/AZMP-modis-bloom.xlsx"
tmp <- read.xlsx2(input_file, sheetName="AZMP-modis-spring-final_updated", as.data.frame=TRUE,
                  header=FALSE, stringsAsFactors=FALSE, startRow=3, colIndex=seq(1,6),
                  colClasses=c("character",rep("numeric", 5)))
names(tmp) <- c("region", "year", "initiation", "amplitude", "duration", "magnitude")
df_data_raw_w <- rbind(df_data_raw_w,
...
input_file <- "~/Projects/ICES_6IZPS_PP/data/Bloom_Metrics/2015-AZMP-spring-bloom.xlsx"
tmp <- read.xlsx2(input_file, sheetName="2015-AZMP_final", as.data.frame=TRUE,
                  header=FALSE, stringsAsFactors=FALSE, startRow=3, colIndex=seq(1,6),
                  colClasses=c("character",rep("numeric", 5)))
names(tmp) <- c("region", "year", "initiation", "amplitude", "duration", "magnitude")
df_data_raw_w <- rbind(df_data_raw_w,
                       tmp %>% dplyr::mutate(., label="2015"))
rm(tmp)
...
## filter for sensor
df_data_filtered_w <- df_data_filtered_w %>%
  dplyr::filter(., (year>=1998 & year<=2008 & label=="seawifs") |
                  (year>=2009 & year<=2014 & label=="modis") |
                  (year==2015 & label=="2015")) %>%
  dplyr::arrange(., region, year)

## reshape data to long format
df_data_filtered_l <- df_data_filtered_w %>%
  dplyr::select(., region, year, initiation, amplitude, duration, magnitude) %>%
  tidyr::gather(., variable, value, initiation, amplitude, duration, magnitude) %>%
...
##--------------------------------------------------------------------------------------------
## save to RData file
ofile <- "~/Projects/ICES_6IZPS_PP/outputs/Bloom_Metrics/Bloom_Metrics.RData"
